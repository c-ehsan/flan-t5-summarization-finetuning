...
class FullFinetune:
    ...
    #tokenizer_function to tokenize dataset
    #our dataset type is pandas so if we want to assign dataset to Trainer we must tokenized dataset
    def tokenizer_funtion(self,example):
        ...
        example["labels"]=self.tokenizer(example["summary"],padding="max_length",truncation=True,return_tensors="pt").input_ids.to(self.device)
        return example

    def tokenized_datasets_func(self):
        self.tokenized_datasets=self.dataset.map(self.tokenizer_funtion,batched=True)
    ...
    #Training Args 
    def training_args_func(self,output_dir_path,per_device_train_batch_size:int,per_device_eval_batch_size:int,epochs:int,gradient_accumulation_steps:int,learning_rate:float,logging_steps:int,weight_decay:int):
